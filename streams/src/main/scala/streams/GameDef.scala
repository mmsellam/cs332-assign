...
trait GameDef {
  ...
  case class Pos(x: Int, y: Int) {
    ...
    def sameAs(obj: Pos): Boolean = x == obj.x && y == obj.y
  }
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = {
      (up, Up) :: (down, Down) :: (right, Right) :: (left, Left) :: Nil
    }
     ...
     */
    def isStanding: Boolean = b1.sameAs(b2)
    ...
    def sameAs(other: Block) =
      (b1.sameAs(other.b1) && b2.sameAs(other.b2)) ||
        (b1.sameAs(other.b2) && b2.sameAs(other.b1))
